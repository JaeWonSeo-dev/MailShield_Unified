...
def load_phishing_email_csv(data_dir: str, max_samples: Optional[int] = None) -> pd.DataFrame:
    """
    phishing_email.csv: text_combined/label 또는 Hugging Face Email Text/Email Type 형식.
    """
    ...
    for _, row in df.iterrows():
        if "label" in df.columns:
            label = int(row["label"])
            body = str(row.get("text_combined", "") or row.get("body", "") or "")
        else:
            email_type = str(row.get("Email Type", "") or "").strip().lower()
            label = 0 if "safe" in email_type else 1
            body = str(row.get("Email Text", "") or "")
        records.append({
            ...
            "reply_to":   "",
            "urls_raw":   "",
            "label_type": "ham" if label == 0 else "phishing",
            "label":      label,
        })
...
def load_big_spam_ham_phish(data_dir: str, max_samples: Optional[int] = None) -> pd.DataFrame:
    """locuoco/the-biggest-spam-ham-phish-email-dataset-300000: label,text."""
    csv_path = Path(data_dir) / "big_spam_ham_phish" / "df.csv"
    if not csv_path.exists():
        raise FileNotFoundError(f"Not found: {csv_path}")

    df = pd.read_csv(csv_path, nrows=max_samples)
    records = []
    label_type_map = {0: "ham", 1: "phishing", 2: "spam"}
    for _, row in df.iterrows():
        raw_label = int(row["label"])
        label_type = label_type_map.get(raw_label, "unknown")
        if label_type == "unknown":
            continue
        records.append({
            "source": "big_spam_ham_phish",
            "subject": "",
    ...
    logger.info(f"Loaded {len(records)} rows from big_spam_ham_phish/df.csv")
    return pd.DataFrame(records)
...
def load_all_datasets(
    ...
    loaders = [
        ...
        ("big_spam_ham_phish", load_big_spam_ham_phish),
    ]
    ...
    combined["_dedupe_text"] = (
        combined["subject"].fillna("").astype(str).str.lower().str.strip()
        + "\n"
    ...
    logger.info(
        ...
        f"Removed duplicates: {before_dedupe - len(combined)}\n"
